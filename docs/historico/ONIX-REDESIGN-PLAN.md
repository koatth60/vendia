...
- [x] **Fase 0** — tokens, tema claro/oscuro, conmutador con anti-flashazo.
- [x] **Fase 1** — esqueleto: barra lateral de 248px, barra superior, subnav en
      píldoras. Las 27 variables viejas de `admin.css` quedaron como alias de los
      tokens, así que TODO el panel (no solo el chrome) responde al tema.
- [x] **Responsive** — auditoría automática a 360/390/768/1024/1440 con
      `design/audit-responsive.mjs`. Arreglados: cabecera del chat (el nombre
      largo montaba a "Cerrar venta"/"Tomar control"), `select` que medía 477px
      por su opción más larga, `.actions-row` sin envolver, casillas de 13px sin
      blanco táctil, franja de usuario con hueco en móvil. Cinco anchos limpios.
- [x] **Fase 2** — Inicio: filas de atención, cifras del mes en monoespaciada,
      accesos rápidos con iconos SVG.
- [ ] Fase 3 — CRM (Bandeja, Clientes, Pedidos)
- [ ] Fase 4 — Catálogo
- [ ] Fase 5 — Bot
- [ ] Fase 6 — Negocio y Analytics (acá entran los gráficos)
- [ ] Fase 7 — Auth
- [ ] Fase 8 — Landing en oscuro
- [ ] Fase 9 — Marca (SVG + favicon a 16/32/48)

### Cómo volver a correr la auditoría

`design/audit-responsive.mjs` carga `public/admin/index.html`, le saca los
scripts, destapa todos los paneles a la vez y mide desbordes, solapes y blancos
táctiles en cinco anchos. Necesita Playwright:
...
Corrélo al cerrar cada fase. Es la única forma honesta de decir "es responsive":
mirarlo en el navegador a un solo ancho no lo prueba.
