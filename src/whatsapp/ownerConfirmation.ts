...
export async function sendConfirmationLadder(
  ...
  const template = await sendAlertToOwner(businessId, credentials, ownerPhone, text);
...
export async function askOwnerToConfirmSale(params: {
  ...
  const outcome = await deliverAndRecord(params.businessId, params.conversationId, params.credentials, business.contactPhone, text);
...
async function deliverAndRecord(
  ...
  const outcome = await sendConfirmationLadder(businessId, credentials, ownerPhone, text);
...
export async function retrySaleConfirmation(params: {
  ...
  const outcome = await deliverAndRecord(params.businessId, params.conversationId, params.credentials, params.ownerPhone, text);
  await prisma.conversation.update({
    where: { id: params.conversationId },
    data: { pendingConfirmationRemindedAt: new Date(), pendingConfirmationAttempts: params.attempt },
  });
  return outcome;
}
